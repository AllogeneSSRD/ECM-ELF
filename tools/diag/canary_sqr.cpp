...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < trials; t++) {
        ...
        /* sqr canonicity/correctness (raw comparison, no mpz_mod masking) */
        {
            std::vector<uint64_t> S(lw);
            ifma_mont_sqr(S.data(), A.data(), &c);
            for (unsigned lane = 0; lane < 8; lane++) {
                mpz_t ra, rr, ww;
                mpz_inits(ra, rr, ww, NULL);
                raw_to_mpz(ra, A.data(), n, lane);
                raw_to_mpz(rr, S.data(), n, lane);
                mpz_mul(ww, ra, ra); mpz_mod(ww, ww, N);
                if (mode == IFMA_FIELD_MONT) { mpz_mul(ww, ww, Rinv); mpz_mod(ww, ww, N); }
                if (mpz_cmp(rr, N) >= 0) { noncanon++; if (noncanon <= 3) gmp_printf("  SQR NON-CANONICAL lane=%u raw=%Zx\n", lane, rr); }
                if (mpz_cmp(rr, ww) != 0) { wrong++; if (wrong <= 3) gmp_printf("  SQR WRONG lane=%u\n    want=%Zx\n    raw =%Zx\n", lane, ww, rr); }
                mpz_clears(ra, rr, ww, NULL);
            }
        }
        for (unsigned lane = 0; lane < 8; lane++) {
