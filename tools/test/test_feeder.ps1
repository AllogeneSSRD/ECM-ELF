...
Copy-Item "$alt\e0000991_c000001.tmp" "$local\e0000991_c000009.tmp" -Force
Write-Host "  local .tmp: $((Get-ChildItem $local -Filter '*.tmp' | ForEach-Object { $_.Name }) -join ', ')"
...
# candidate is e0000991_c000009.tmp, whose B1=200000 does not match the
...
Get-ChildItem $local -Filter "*.tmp" | Where-Object { $_.Name -ne "e0000991_c000009.tmp" } |
...
$mismatch_ok = (-not (Test-Path "$p95\worktodo.add")) -and (Test-Path "$local\e0000991_c000009.tmp")
Write-Host ""
Write-Host "############ RESULT ############"
