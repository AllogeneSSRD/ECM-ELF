...
/* r -= N where (r, top) >= N; two passes so the originals are still available */
static void soa_cond_sub(uint64_t *r, __m512i top, const ifma_ctx_t *mc)
{
    ...
    for (size_t i = 0; i < n; i++) {
        const __m512i d = _mm512_sub_epi64(_mm512_sub_epi64(ld(r + 8 * i), ld(nb + 8 * i)), borrow);
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        borrow = _mm512_maskz_set1_epi64(_mm512_movepi64_mask(d), 1);
        st(r + 8 * i, _mm512_mask_blend_epi64(need, rk, _mm512_and_si512(d, mask)));
    }
}
...
static void soa_add(uint64_t *r, const uint64_t *a, const uint64_t *b, const ifma_ctx_t *mc)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        st(r + 8 * i, _mm512_and_si512(s, mask));
    }
...
static void soa_neg(uint64_t *r, const uint64_t *a, const ifma_ctx_t *mc)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        const __m512i d = _mm512_sub_epi64(_mm512_sub_epi64(nk, ai), borrow);
        borrow = _mm512_maskz_set1_epi64(_mm512_movepi64_mask(d), 1);
...
static void soa_sub(uint64_t *r, const uint64_t *a, const uint64_t *b, uint64_t *t,
                    const ifma_ctx_t *mc)
{
    soa_neg(t, b, mc);
    soa_add(r, a, t, mc);
...
static void xz_dbl(const soa_xz &r, const soa_xz &p, const uint64_t *a24,
                   uint64_t *A_, uint64_t *B_, uint64_t *E_, uint64_t *t, const ifma_ctx_t *mc)
{
    soa_add(A_, p.X, p.Z, mc);
    ifma_mont_sqr(A_, A_, mc);
    soa_sub(B_, p.X, p.Z, t, mc);
    ifma_mont_sqr(B_, B_, mc);
    soa_sub(E_, A_, B_, t, mc);                    /* E = 4XZ */
    ifma_mont_mul(r.X, A_, B_, mc);
    ifma_mont_mul(t, a24, E_, mc);
    soa_add(t, t, B_, mc);                         /* B + a24*E */
...
static void xz_add(const soa_xz &r, const soa_xz &p, const soa_xz &q, const uint64_t *xdiff,
                   ...
                   uint64_t *t4, uint64_t *t5, const ifma_ctx_t *mc)
{
    soa_add(t0, p.X, p.Z, mc);
    soa_sub(t1, p.X, p.Z, t5, mc);
    soa_add(t2, q.X, q.Z, mc);
    soa_sub(t3, q.X, q.Z, t5, mc);
    ifma_mont_mul(t4, t0, t3, mc);
    ifma_mont_mul(t5, t1, t2, mc);
    soa_add(t0, t4, t5, mc);
    soa_sub(t1, t4, t5, t2, mc);
    ifma_mont_sqr(t4, t0, mc);
    ifma_mont_sqr(t5, t1, mc);
    ifma_mont_mul(r.Z, t5, xdiff, mc);
    memcpy(r.X, t4, 8 * mc->n * sizeof(uint64_t));
}
                         ...
                         mpz_t *out_x, mpz_t *out_gcd)
{
    ...
    xz_dbl(R1, R0, a24, A_, B_, E_, tt, mc);

    soa_xz *p0 = &R0, *p1 = &R1, *pt = &T;
    for (size_t i = 1; i < nbits; i++) {           /* bits[0] is the implicit top bit */
        if (bits[i]) {
            xz_add(*pt, *p1, *p0, xdiff, t0, t1, t2, t3, t4, t5, mc);
            xz_dbl(*p1, *p1, a24, A_, B_, E_, tt, mc);
            soa_xz *sw = p0; p0 = pt; pt = sw;
        } else {
            xz_add(*pt, *p0, *p1, xdiff, t0, t1, t2, t3, t4, t5, mc);
            xz_dbl(*p0, *p0, a24, A_, B_, E_, tt, mc);
