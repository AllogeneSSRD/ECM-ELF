...
## 22. B1 = 260e6 的十秒：分段筛 + 分块累加 + 带校验的缓存（2026-09-26，用户实测报障）

**报障**：最近实测 B1 = 260e6 时"生成 K 要 20~30 秒"（K = stage-1 指数 `s = torsion*lcm(1..B1)`，
在每个任务开始前都要算一次）。本节记录定位、两处算法修改、以及"缓存 + 校验"方案。

### 22.1 先量，再改（阶段归因）

用 `.bench_tmp/obj/bench_lcm.cpp`（隔离出 `ecm_build_lcm_exponent`，不跑 stage 1、不碰 GPU）：

| B1 | s 的位长 | 修改前 | 修改后 |
|---|---|---|---|
| 1e6 | 1.44 Mbit | 0.015 s | 0.012 s |
| 1e7 | 14.4 Mbit | 0.20 s | 0.17 s |
| 1e8 | 144.3 Mbit | 5.2–5.4 s | **2.95 s** |
| **2.6e8** | **375.1 Mbit（47 MB）** | **16.5 s** | **10.7 s** |

`-DECM_EXP_TIMING` 把 10.7 s 拆开：**筛法只占 0.14 s**（我最初猜错的方向）、每素数累加 ~0.3 s、
**树内折叠 7.5 s、最终归并 2.8 s**。剩下的是**固有的**大数乘法代价：单独测 GMP
`mpz_mul` 两个 187.5 Mbit 的数 = **0.871 s**，而 375 Mbit 乘积的树的上几层就要做几次这种乘法
（50 Mbit×50 Mbit 只要 0.197 s ⇒ 这个 GMP 的 FFT 是开着的，但每层成本几乎不随规模下降）。

### 22.2 与 prmers 的对照（用户指定：`/PrMers-main/src/modes/RunEcmTwistedEdwards.cpp` "Building K"）

prmers 的做法（1399–1532 行）与我们的结论**一致**：① `ecm_sieve_base_primes(sqrt(B1))` + 奇数
分段筛 `ecm_segmented_primes_odd`；② 逐素数取最高次幂 `while (mm <= B1/q) mm *= q`，乘进一个
**固定大小（32768 bit）的块**；③ 块用**两两配对**归并（`chunks[j] * chunks[j+1]`），不是顺序乘进大数。
它也有 `Building K done/total | primes/s | ETA` 的进度条 —— 说明"这段要几十秒"在两边都是已知成本。
我们的实现与它的差别只在块大小（512 bit vs 32768 bit，我们实测小块更快）和用二进制计数器而不是
块向量；**没有更快的现成算法可抄**：prmers 同样是 ~10 s 量级。

### 22.3 本次落地的四处改动（都在 `src/core/ecm_stage1_exp.cpp`）

1. **奇数分段筛**（2^19 个奇数/段 = 512 KiB 标志位，留在 L2 里）：原来 `std::vector<char> sieve(B1+1)`
   在 2.6e8 时是 **260 MB 的全范围随机写**。
2. **每素数的手写小累加器**：把素数乘进一个 ≤512 bit 的 32 位 limb 数组（`mul_u32` 原地乘），
   而不是每个素数一次 `mpz_mul_ui` —— GMP 那次调用内部要"分配临时数组 + 拷回"（实测 ~600 ns/素数，
   单这一项就是 8.5 s）；手写版一次约 10–20 ns。块满了才折进树。
3. **每个素数省掉一次除法**：只有 `p <= sqrt(B1)` 才可能出现 >1 次幂（2.6e8 时只有 2001 个），
   其余素数直接贡献自己。
4. **最终归并改成两两配对**：原来从最大槽位顺序乘（累加器长到 47 MB 还要再乘小槽位），实测这一项
   就要 6.3 s。

### 22.4 缓存 + 校验（用户要求"引入缓存文件时应该予以校验"）

`src/core/ecm_stage1_exp_cache.{h,cpp}`：`s` 以二进制落盘（默认目录 = exe 目录，
`--exp-cache <dir>` / `ecm.ini: exp_cache = <dir|off>` 可改；B1 < 1e6 不缓存）。

**校验链（任何一项不过就忽略并重建，绝不使用可疑缓存）**：

| # | 检查 | 能挡住什么 |
|---|---|---|
| 1 | 魔数 / 格式版本 / 语义 id / **B1 与 torsion 精确相等** | 张冠李戴、格式变更 |
| 2 | 位长落在 `1.4427*B1` 的 ±1% 带内 + 载荷长度与声明字数一致 | 截断、明显错的 B1 |
| 3 | FNV-1a 64 + 加和校验（**覆盖头部字段 + 载荷**）| 位翻转、头部被改（改名/改字段都会破坏哈希）|
| 4 | **语义整除**：必须能被 (a) 前 32 个 ≤B1 的素数最高次幂之积、(b) **B1 下方最近 32 个素数**之积整除 | (b) 是"这份载荷确实属于这个 B1"的证明：为更小 B1 生成的缓存在这里必然失败 |

写入用 `*.tmp` + 原子替换（Windows 走 `MoveFileExA(MOVEFILE_REPLACE_EXISTING)`），并发的两个进程
各写各的临时文件、结果都是合法文件。

**实测（`.bench_tmp/obj/bench_exp_cache.cpp`，10/10 通过）**：

| 场景 | 结果 |
|---|---|
| B1 = 2.6e8 首次（建 + 写盘） | **10.96 s** |
| B1 = 2.6e8 第二次（校验 + 载入） | **0.26 s（42×）** |
| 载荷翻转一个字节 | 拒绝：`payload/header checksum mismatch` |
| 头部被改 | 拒绝：截断/校验失败 |
| 文件截断 | 拒绝：`payload truncated` |
| 用别的 B1/torsion 打开 | 拒绝：`cached parameters differ` |
| **头部标 B1+1000、载荷是 B1**（头与哈希自洽） | 拒绝：`not divisible by the primes just below B1` ✓ 语义检查在这里是**唯一**能挡住它的 | 
| 缓存关闭（`--exp-cache off`） | 每次重建，行为与修改前一致 |

驱动侧会打印一行状态，例如
`stage1 exponent : s_bits=375102575 (lcm(1..2.6e+08) x 1) [cache hit (0.26 s, ...)]`。

### 22.5 本轮踩到的坑（都真实发生过）

1. **分段筛的右端不能写成 `B1 - span`**：`uint64_t` 下溢会在 B1 < span 时把**大于 B1 的素数**放进来
   （B1=1e6 实测多出 **+70,719 bit**，正好是 (1e6, 1.048e6] 区间的素数）。改成先 `lo+span` 再判
   `if (hi > B1) hi = B1;`，并用 bits 对比原实现逐位相等来验证。
2. **"块满时的进位"不能丢**：手写累加器一开始在 `n == EXP_BLOCK_LIMBS` 时把进位丢掉 ⇒ 静默丢素数幂
   （1e6 实测 1,437,860 bit ≠ 1,442,099 bit）。现在改成**乘之前**用 `needs_flush()` 精确保留进位空间
   （用位长估计会短一点，同样会丢）。
3. **二进制计数器的进位必须走临时变量**：我把 `slot[j]` 直接乘进 `slot[j+1]`（"省一次拷贝"），破坏了
   "slot j 装 2^j 个块"的不变式 ⇒ 值越滚越大、复杂度变平方，**跑 10 分钟没结束**（用户发现并叫停）。
   已恢复为"累乘到临时变量、落到第一个空槽"，并在代码里写明"不要再改成原地进位"。
4. **`<windows.h>` 把 `small` 定义成 `char`**（rpcndr.h 的老兼容宏）：`static const uint64_t small[]`
   变成 `uint64_t char[]`，报 `C2628: "uint64_t"后面接"char"是非法的`。改名 `mr_bases`。
5. **单文件语法检查救了两次**：`ecm_mont_cpu.h` 用了 `std::string` 却没包含 `<string>`（以前靠传递包含
   蒙对了）。改完头文件签名后 `cl /Zs` 每个 TU 过一遍，比全量构建便宜得多。

### 22.6 仍未做

* **驱动级端到端**（`--method mont` + 真实 B1 的那一行日志）还没跑：本机 dev 构建的 `ecm.exe` 是
  **OpenCL** 驱动（`mode: cpu-stub`），Montgomery 路径在 CPU 构建（`build_vs18`）里 ⇒ 需要构建
  `build_vs18` 才能看到 `[cache hit ...]` 落在真实任务上。模块本身已由单元测试覆盖（22.4 表）。
* 首跑仍是 ~10.7 s。要再降只能：换一个更快的 GMP（本机 `M(187.5 Mbit)=0.871 s`），或**不materialize
  整个乘积**（GPU 侧只按位顺序消费 `s_bits`，理论上可以流式生成）——后者是架构级改动，未评估。
* 缓存目前按 (B1, torsion) 存整份 47 MB 文件；如果以后 B1 取值很多，需要加 LRU/上限。
