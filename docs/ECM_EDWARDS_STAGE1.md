...
**M1 未做（留给 M2）**：
- **平方专用内核**：现在 `ifma_mont_sqr` 直接调 `ifma_cios(a,a)`，而标量侧 `mpn_sqr` 比
  `mpn_mul_n` 快 ~18%，所以 sqr 那一列的比值（2.2–2.9×）**低估了** SIMD 的潜力；做对称
  平方（少一半 (i,j) 对）能把混合比值再抬 ~10–15%。
- **分块/寄存器窗口版**：当前累加器在 L1 里逐列 load/store。n=154 时 u 掉到 1.34–1.83，
  值得按 ~20 列一块、B+1 个活跃累加器常驻寄存器重写一版（§13.3 的窗口分析仍然适用）。
- M2：Edwards 点层 SoA（dbl/add/add_affine）、字典 SoA、与标量路径的 u/gcd/因子一致性。
